...
-spec parse_response(json()) -> [{id(), response()}].
...
-spec batch_call([{method(), params()}], transportfun(),
...
-spec parse_single_response(json()) -> {id(), response()}.
parse_single_response(Response) when is_map(Response) ->
    ...
    Reply = case {Result, Error} of
        ...
        {undefined, {ErrorProplist}} ->
            ...
            ErrorTuple = case maps:get(<<"data">>, ErrorProplist) of
                undefined ->
